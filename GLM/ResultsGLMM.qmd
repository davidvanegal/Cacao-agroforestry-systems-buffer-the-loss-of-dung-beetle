...
```{r echo=FALSE}
# Read data
GLMMDB <- read.csv("GLM_DungBeetles.csv", header = T, sep = ";")
...
for (i in 4:19) {
  x <- names(GLMMDB)[i]
  modNull <- as.formula(paste(x, paste("1"), 
                          sep=" ~ "))
  mod <- as.formula(paste(x, paste("VegCover"), 
                          sep=" ~ "))
  DBNull <-  glm(formula = modNull, data = GLMMDB, family = gaussian())
  DB <-  glm(formula = mod, data = GLMMDB, family = gaussian())
  anvNull <- anova(DBNull, DB, test = "Chisq")
  tukDB <- glht(DB, linfct = mcp(VegCover = "Tukey"))
